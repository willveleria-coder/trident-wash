...
const ROW_1: Item[] = [
  ...
  { text: '5 stars on Google', variant: 'cyan-badge', icon: Star },
  ...
  { text: 'Five-star reviews', variant: 'black-chip', icon: Star },
...
const ROW_2: Item[] = [
  ...
  { text: 'Fixed-price quotes', variant: 'plain', icon: Zap },
  { text: 'Same-day quotes', variant: 'black-chip', icon: Zap },
  { text: 'Photos before & after', variant: 'plain', icon: Sparkles },
  { text: 'Public Liability Insurance', variant: 'white-stamp', icon: Shield },
...
function MarqueeItem({ item, dark = false }: { item: Item; dark?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-5 lg:gap-7 px-5 lg:px-7 shrink-0">
      {item.variant === 'plain' && (
        <span className={`font-display text-2xl lg:text-4xl tracking-tight flex items-center gap-3 ${dark ? 'text-yellow-400' : 'text-slate-900'}`}>
      ...
      <Star className={`w-4 h-4 lg:w-5 lg:h-5 ${dark ? 'fill-yellow-400 text-yellow-400' : 'fill-slate-900 text-slate-900'}`} />
    </div>
  );
}